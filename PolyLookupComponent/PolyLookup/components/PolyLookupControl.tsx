...
const Body = ({
  ...
  const [initialOutputSynced, setInitialOutputSynced] = React.useState(false);
  ...
  useEffect(() => {
    if (!initialOutputSynced && isLoadingSelectedItemsSuccess && onChange) {
      onChange(
        selectedItems?.map((i) => {
          return {
  ...
  }, [initialOutputSynced, isLoadingSelectedItemsSuccess, onChange]);
  ...
  const filterQuery = useMutation({
    mutationFn: ({ searchText, pageSizeParam }: { searchText: string; pageSizeParam: number | undefined }) => {
      ...
      if (!lookupView && metadata?.associatedView.querytype === 64) {
        ...
        const fetchXmlTemplate = Handlebars.compile(fetchXml);
